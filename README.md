...
This makes a `SecureString` virtually useless for an unattended script that needs to handle secrets and difficult to use for less secure but sensitive information (e.g. an embedded API key) as it doesn't provide easy string convertors as it considered unsafe.
